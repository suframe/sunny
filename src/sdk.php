 ...
 *
 */
sunny_ConsoleOut("欢迎使用内网穿透 sunny-php v1.38\r\nCtrl+C 退出");
...
$Tunnels = array();
// 根据隧道id请求接口获取隧道信息
$serverArr = sunny_ngrok_auth($clientid);
...
$pingtime = 0;
//建立隧道协议
$mainsocket = sunny_connectremote($seraddr, $port);
if ($mainsocket) {
    $socklist[] = array('sock' => $mainsocket, 'linkstate' => 0, 'type' => 1);
}
//注册退出执行函数
register_shutdown_function('sunny_shutdown');
while ($recvflag) {
    //重排
    ...
    //检测控制连接是否连接.
    if ($mainsocket == false) {
        $ip = sunny_dnsopen($seraddr);//解析dns
        if (!$ip) {
            sunny_ConsoleOut('连接ngrok服务器失败.');
            sleep(1);
            continue;
        }
        $mainsocket = sunny_connectremote($ip, $port);
        if (!$mainsocket) {
            sunny_ConsoleOut('连接ngrok服务器失败.');
            sleep(10);
            continue;
    ...
    if ($pingtime + 25 < time() && $pingtime != 0) {
        sunny_sendpack($mainsocket, sunny_Ping());
        $pingtime = time();
    }
    //重新赋值
    $readfds = array();
    ...
    if ($res === false) {
        sunny_ConsoleOut('sockerr', 'debug');
    }
    //有事件
    if ($res > 0) {
        foreach ($socklist as $k => $sockinfo) {
            ...
            if (in_array($sock, $readfds)) {
                ...
                if ($sockinfo['type'] == 1 || ($sockinfo['type'] == 2 && $sockinfo['linkstate'] == 1)) {
                    ...
                    $len = sunny_tolen1($lenbuf);
                    if (strlen($allrecvbut) >= (8 + $len)) {
                        $json = substr($allrecvbut, 8, $len);
                        sunny_ConsoleOut($json, 'debug');
                        $js = json_decode($json, true);
                        //远程主连接
                        if ($sockinfo['type'] == 1) {
                            if ($js['Type'] == 'ReqProxy') {
                                $newsock = sunny_connectremote($seraddr, $port);
                                if ($newsock) {
                            ...
                            if ($js['Type'] == 'AuthResp') {
                                ...
                                $pingtime = time();
                                sunny_sendpack($sock, sunny_Ping());
                                foreach ($Tunnels as $tunnelinfo) {
                                    //注册端口
                                    sunny_sendpack($sock,
                                        sunny_ReqTunnel($tunnelinfo['protocol'], $tunnelinfo['hostname'],
                                            $tunnelinfo['subdomain'], $tunnelinfo['httpauth'], $tunnelinfo['rport']));
                                }
                            }
                            if ($js['Type'] == 'NewTunnel') {
                                if ($js['Payload']['Error'] != null) {
                                    sunny_ConsoleOut('隧道建立失败:' . $js['Payload']['Error']);
                                    sleep(30);
                                } else {
                                    sunny_ConsoleOut('隧道建立成功:' . $js['Payload']['Url']);
                                }
                            }
                        }
                        //远程代理连接
                        if ($sockinfo['type'] == 2) {
                            ...
                            if ($sockinfo['linkstate'] == 1) {
                                if ($js['Type'] == 'StartProxy') {
                                    $loacladdr = sunny_getloacladdr($Tunnels, $js['Payload']['Url']);
                                    $newsock = sunny_connectlocal($loacladdr['lhost'], $loacladdr['lport']);
                                    if ($newsock) {
                                        $socklist[] = array(
                                            'sock' => $newsock,
                                            'linkstate' => 0,
                ...
                if ($sockinfo['type'] == 3 || ($sockinfo['type'] == 2 && $sockinfo['linkstate'] == 2)) {
                    sunny_sendbuf($sockinfo['tosock'], $sockinfo['recvbuf']);
                    $sockinfo['recvbuf'] = '';
                    $socklist[$k] = $sockinfo;
                }
            }
            ...
            if (in_array($sock, $writefds)) {
                if ($sockinfo['linkstate'] == 0) {
                    if ($sockinfo['type'] == 1) {
                        sunny_sendpack($sock, sunny_NgrokAuth(), false);
                        $sockinfo['linkstate'] = 1;
                        $socklist[$k] = $sockinfo;
                    }
                    if ($sockinfo['type'] == 2) {
                        sunny_sendpack($sock, sunny_RegProxy($ClientId), false);
                        $sockinfo['linkstate'] = 1;
                        $socklist[$k] = $sockinfo;
                    }
                    if ($sockinfo['type'] == 3) {
...
function sunny_sendpack($sock, $msg, $isblock = true)
{
    ...
    fwrite($sock, sunny_lentobyte1(strlen($msg)) . $msg);
    if ($isblock) {
...
function sunny_ngrok_auth($clientid)
{
    ...
    if (!$fp) {
        sunny_ConsoleOut('连接认证服务器: https://www.ngrok.cc 错误.');
        sleep(10);
        exit();
    }
    ...
    if ($authData['status'] != 200) {
        sunny_ConsoleOut('认证错误:' . $authData['msg'] . ' ErrorCode:' . $authData['status']);
        sleep(10);
        exit();
    }
    sunny_ConsoleOut('认证成功,正在连接服务器...');
    //设置映射隧道,支持多渠道[客户端id]
    sunny_ngrok_adds($authData['data']);
...
{
    global $mainsocket;
    sunny_sendpack($mainsocket, 'close');
    fclose($mainsocket);
}
